...
Neue Reihenfolge für angereicherte Daten:
...
1. Ohne jeden API-Key zeigt die Detailansicht Flugzeugtyp, Halter und —
   sofern bekannt — Start- und Zielflughafen.
2. Routendaten werden nachweislich nicht auf Platte geschrieben.
3. Bei 30+ sichtbaren Flugzeugen entsteht keine Anfrageflut: pro
   Poll-Zyklus nur die konfigurierte Anzahl, gedrosselt.
4. Netzwerkausfall während laufender Anreicherung führt weder zu Absturz
   noch zu spürbarem Ruckeln in der Sweep-Animation.
5. adsbdb ist in App, Portal, README und Anforderungen als Quelle genannt.
6. Alle Tests grün.
